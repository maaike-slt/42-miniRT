...
/*   Updated: 2025/06/03 20:43:31 by msloot           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	parse_line_arg(t_env *env, const t_line_arg *la)
{
	...
	if (ft_strcmp(la->split[0], "cy") == 0)
		return (parse_cylinder(env, la));
	if (ft_strcmp(la->split[0], "tr") == 0)
		return (parse_triangle(env, la));
	if (ft_strcmp(la->split[0], "sq") == 0)
		return (parse_square(env, la));
